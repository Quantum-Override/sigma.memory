...
## Project-Local Test Runner (`rtest`)

The `rtest` script provides a local test runner for sigma.memory that intelligently searches for tests in subdirectories and supports running individual tests or entire test suites.

Make `rtest` executable (already done): `chmod +x rtest`

### Test Specification Formats

Run with: `./rtest <test_spec> [options]`

The test specification can be:
- **test_name**: Run specific test (searches all subdirectories)
- **subdir/test_name**: Run specific test in subdirectory
- **subdir**: Run all tests in subdirectory

The script automatically searches for test files in these subdirectories:
...
The script sources `config.sh` for all build configuration including:
- `CC`, `CFLAGS`, `TST_CFLAGS` - Compiler and flags
- `BUILD_DIR`, `SRC_DIR`, `TEST_DIR` - Directory paths
- `REQUIRES` - Required library dependencies
- `VALGRIND_ENABLED`, `VALGRIND_OPTS` - Memory checking settings

### Options

- `--valgrind`: Run the test(s) with valgrind for memory leak detection
- `--help`, `-h`: Show help message

### Examples

```bash
# Run specific test by name (searches all subdirs)
./rtest nodepool_growth

# Run specific test with explicit path
./rtest unit/nodepool_growth

# Run all tests in unit/ subdirectory
./rtest unit

# Run with valgrind
./rtest unit/nodepool_growth --valgrind

# Run all unit tests with valgrind
./rtest unit --valgrind
...
When running all tests in a directory, `rtest` provides a summary:
...
### Differences from ctest

- **Local**: `rtest` is project-specific and lives in the repository root
- **Subdirectory support**: Automatically finds tests in `test/unit/`, `test/integration/`, etc.
- **Batch execution**: Can run all tests in a subdirectory with one command
- **Config-driven**: Uses local `config.sh` for all build settings
- **Simplified**: Focused on sigma.memory project structure and requirements
- **Portable**: No dependency on `/usr/local/scripts/` global installation
...
### Per-Test REQUIRES and Include Overrides

You can override the global `REQUIRES` and add test-specific include paths for individual tests using two associative arrays in `config.sh`:

```bash
# Default REQUIRES for most tests
REQUIRES=("sigma.core.alloc")

# Per-test REQUIRES overrides (optional)
declare -A TEST_REQUIRES=(
    ["text_memory_compat"]="sigma.memory"  # Override for this test
)

# Per-test include paths (optional)
declare -A TEST_INCLUDES=(
    ["text_memory_compat"]="-I../sigma.memory/include"  # Additional includes
)
```

- If `TEST_REQUIRES` is defined and a test name is present, only those packages are linked for that test.
- If `TEST_INCLUDES` is defined and a test name is present, those flags are appended to the test's compile flags.
- If not defined, or the test is not present, the global `REQUIRES` and default includes are used.

**Example:**

```bash
$ ctest text_memory_compat
Using test-specific REQUIRES: sigma.memory
Using test-specific includes: -I../sigma.memory/include
Compiling test/test_text_memory_compat.c -> build/test/test_text_memory_compat.o
Linking build/test/test_text_memory_compat
=== Running text_memory_compat ===
[Tests pass]
```

This allows you to test compatibility with different packages or include paths on a per-test basis, without affecting other tests or the global configuration.