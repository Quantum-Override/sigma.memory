...
- **NodePool**: Separate 24KB pool (1024 nodes) for B-Tree allocation metadata
...
**NodePool (24KB initial, separate mmap):**
- 1024 nodes × 24 bytes = 24,576 bytes
- Fixed array of `sc_node` structures
- Node index 0xFFFF = null sentinel
- Growable via remap (transparent to all code)

**User Pages (custom, default 4KB each, per-slab):**
...
**SYS0** = RECLAIMING (no metadata, no block headers)  
