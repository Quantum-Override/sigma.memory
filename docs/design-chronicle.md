...
## Chapter 10: Janky Instability — NodePool B-tree Index Corruption (v0.2.4)

### The Situation

**Date:** March 11, 2026  
**Context:** All previous test suites passing (18 unit, 2 integration, 2 validation). A new `test_janky.c` suite was written to stress mixed-scope usage: frames opened in non-current scopes, routing correctness across arena/SLB0 boundaries, frame lifetime vs. allocation lifetime.  
**Symptom:** `test_page_release` PRL-05 — page_count 16 → 16 when it should go 17 → 16 after disposing the last allocation on a dynamic page.

The suite was named **"janky"** because the patterns it tests are the uncomfortable, awkward interactions that don't appear in polished examples: opening a frame in SLB0 while an arena is current, allocating in both scopes simultaneously, disposing an arena that still has an active frame. Real production code is janky. The test suite had to be too.

---

### The Symptom: PRL-05 "Count 16 → 16"

PRL-05 exercised the full dynamic-page lifecycle:
1. Fill 16 initial SLB0 pages to capacity
2. Allocate one more object → triggers new dynamic page (page_count: 16 → 17)
3. Free everything *except* that last object
4. Dispose the last object → page should be released (page_count: 17 → 16)

PRL-05 printed: `Expected page release; page_count: 16 → 16`.

The page count went up on creation and then **refused to come back down**.

---

### The Investigation

**Stage 1: Skip-list diagnostic**

First suspect was the skip list failing to locate the dynamic pointer during dispose, causing an early return before `alloc_count` could be decremented:
...
The skip list returned `rc = -1` for the "dynamic" pointer — it genuinely could not find the page that should contain it.

**Stage 2: Chain walk**

Added a walk of the entire SLB0 page chain. Confirmed: 16 pages total, all in the expected address range, none above the dynamic threshold. The dynamic page existed physically (we just allocated from it) but the chain appeared to have only 16 entries. How?

**Stage 3: B-tree dynamic-address detector — the smoking gun**

Added a detector in `slb0_alloc`'s Tier 2 (free-block reuse) path: if a `btree_page_find_free` call on an *initial* page returned a free-block address that was above the dynamic threshold, log it:

```
[SLB0-DBG] page_idx=10 btree returned dynamic-range addr=0x7f3a84210000 (dyn_thresh=0x7f3a84008000)
```

Page 10 was an initial page. Its B-tree was returning free blocks with addresses in the dynamic range — addresses that **did not exist** in any valid page. We were handing out phantom memory.

With 29 entries examined, they all showed addresses at sequential 256-byte offsets starting at some dynamic-range base. This was not fragmentation or coalescing gone wrong. These were **ghost nodes** — nodes that had no business being in an initial page's B-tree.

---

### Root Cause: mremap Growth Corrupts All Stored B-tree Indices

**The NodePool index formula:**

Every B-tree node is stored in the NodePool. Its physical address is computed on demand from its index:

```c
// nodepool_get_btree_node:
usize capacity_nodes = header->capacity / sizeof(sc_node);
usize offset = header->capacity - (capacity_nodes - idx) * sizeof(sc_node);
return (sc_node *)((byte *)header + offset);
```

The formula depends on `header->capacity`. Nodes are packed from the **top** of the pool downward.

**The growth path:**

```c
// nodepool_alloc_btree_node (simplified):
new_capacity = old_capacity * 2;
mremap(pool, old_capacity, new_capacity, MREMAP_MAYMOVE);
// Move existing btree_nodes from old top to new top:
memmove(new_top, old_top, btree_section_size);
header->capacity = new_capacity;
```

After `memmove`, the btree_nodes physically live at the new top. `nodepool_get_btree_node` uses the new `header->capacity` in its formula. So far so good — a freshly allocated or freshly fetched node is correct.

**The corruption:**

Every `page_node.btree_root` field stores a **raw index** into the NodePool. Every `sc_node.left_idx` and `sc_node.right_idx` stores a raw index. These were correct when written. After capacity doubles:

```
old_capacity = 8192,  capacity_nodes = 341
new_capacity = 16384, capacity_nodes = 682

// Index 5 before growth:
offset = 8192  - (341 - 5)  * 24 = 8192  - 8064 = 128  ← correct node

// Index 5 AFTER growth (idx unchanged, capacity changed):
offset = 16384 - (682 - 5)  * 24 = 16384 - 16248 = 136  ← different node!
```

The same stored index maps to a completely different physical node after capacity doubles. The B-trees were not corrupted by stray writes — they were corrupted by the **index space shifting under them**. Every initial page's B-tree now had its `btree_root` pointing at a node that belonged to an old dynamic page's tracking data. When `btree_page_find_free` walked these phantom trees, it found free-block records with dynamic-range addresses accumulated from past allocations.

**Why this was invisible for so long:**

- NodePool growth only happens once enough distinct pages are tracked (16+ in the default configuration)
- The corruption produced phantom *free* blocks, not illegal access — so it silently returned bad addresses without crashing
- The phantom addresses, when looked up in the skip list, returned `rc = -1` (not found), triggering an early return in `slb0_dispose` — no `alloc_count` decrement, no page release, no crash, just a silent wrong answer

---

### Secondary Bug: alloc_count Desync in the Recycled-Block Path

The B-tree corruption diagnosis revealed a second, independent bug in `slb0_alloc`. When the allocator recycled a free block via `btree_page_find_free` (the MTIS Tier 2 path), it never incremented `alloc_count` on the page sentinel:

```c
// BUG: btree recycle path - alloc_count never incremented
addr block = btree_page_find_free(slb0, page_idx, size);
// ... split or use whole block ...
goto exit;  // alloc_count untouched
```

But `slb0_dispose` always decrements `alloc_count`. Each recycle+dispose cycle silently decremented `alloc_count` by 1. After enough cycles a page with live allocations showed `alloc_count = 0`, triggering a premature `munmap` of a page that was still in use.

This bug was present from the initial MTIS implementation and only became detectable once the B-tree corruption was fixed (otherwise the skip-list early return masked the decrement entirely).

---

### The Fix

**Part 1: `btree_reindex_after_growth()`**

After every `mremap+memmove` growth, patch every stored btree_node index by the index-space delta:

```c
static void btree_reindex_after_growth(scope scope_ptr,
                                        usize old_capacity,
                                        usize new_capacity) {
    ...
    // δ = 341 for 8192→16384 growth, 683 for 16384→32768, etc.
    #define REINDEX(field) \
        do { if ((field) != NODE_NULL) (field) = (uint16_t)((field) + delta); } while (0)

    // 1. Scope-level indices
    ...
    // 2. Nodepool header
    nodepool_header *header = (nodepool_header *)scope_ptr->nodepool;
    REINDEX(header->btree_free_head);

    // 3. All page_node.btree_root fields
    byte *pool = (byte *)header;
    usize pa_off = sizeof(nodepool_header);
    while (pa_off < header->page_alloc_offset) {
        page_node *pn = (page_node *)(pool + pa_off);
        REINDEX(pn->btree_root);
        pa_off += sizeof(page_node);
    }

    // 4. All btree_node left/right/chain fields
    usize btree_off = header->btree_alloc_offset;
    while (btree_off < new_capacity) {
        sc_node *n = (sc_node *)(pool + btree_off);
        REINDEX(n->left_idx);
        REINDEX(n->right_idx);
        REINDEX(n->frame_data.next_chunk_idx);  // union covers large_alloc chain too
        btree_off += sizeof(sc_node);
    }
    #undef REINDEX
}
```

Called immediately after the `memmove` in both `nodepool_alloc_btree_node` and `nodepool_alloc_page_node`.

**Part 2: `alloc_count++` in the recycled-block path**

```c
// In slb0_alloc MTIS Tier 2, after btree_page_find_free succeeds:
addr recycle_page_base = pn->page_base;  // capture before potential mremap
// ... split or use whole block ...
page_sentinel recycle_ps = (page_sentinel)recycle_page_base;
recycle_ps->alloc_count++;  // keep in sync with slb0_dispose's decrement
goto exit;
```

Note the pre-capture of `recycle_page_base`: `btree_page_insert` (called during the split path) can trigger `mremap`, which may move `pn`. The `pn` pointer must not be used after the split call — capture the base address first.

---

### The JK-DBG Test Apparatus

Rather than throw away the diagnostic scaffolding, we promoted it to permanent regression tests in `test_janky.c`. Two tests atomize the exact failure conditions:

**JK-DBG-01: B-tree tracks a single SLB0 allocation**  
Verifies the complete tracking contract for one allocation:
- `skiplist_find_containing` must return the correct `page_idx`
- `btree_page_search` must find the allocation on that page
- `page_sentinel.alloc_count >= 1`
- After `dispose`: `alloc_count` decrements

This is the atomic unit. If any of these invariants breaks, we know exactly which layer failed.

**JK-DBG-02: B-tree tracks first alloc on dynamic page; dispose releases page**  
Reproduces the exact PRL-05 failure scenario end-to-end:
- Fill until `page_count` increases (new dynamic page created)
- Confirm skip list finds the dynamic allocation
- Confirm B-tree has an entry for the dynamic allocation
- Free all other allocations
- Dispose the dynamic allocation → `page_count` must decrease

Both tests run **before** JK-01..05 in `init_test`. This ordering matters: JK tests create arenas and allocations that can disturb SLB0's initial clean state. DBG tests require `page_count == 16` at entry. If they ran after JK tests, they would see a dirty SLB0 and produce false results.
...
| File | Change |
|------|--------|
| `src/node_pool.c` | Added `btree_reindex_after_growth()`; call it in both alloc-btree-node and alloc-page-node growth paths; added upper bounds guard in `nodepool_get_btree_node` |
| `src/memory.c` | Added `recycle_page_base` pre-capture; added `recycle_ps->alloc_count++` in recycled-block path |
| `test/unit/test_janky.c` | JK-DBG-01 and JK-DBG-02 added as permanent regression tests; DBG registered before JK-01..05 |
| `test/validation/test_nodepool_growth_validation.c` | NPV-01: `btree_root` sentinel changed from `0x1234` to `NODE_NULL` (live index gets patched by reindex; non-null sentinel was invalid after fix) |
| `test/integration/test_arena_integration.c` | AIT-01: `MAX_ARENAS` 14→13 (sigma.test's own arena correctly persists in slot 2, leaving 13 not 14 free slots) |

---

### Test Suite Results (v0.2.4)

| Suite | Tests | Result | Notes |
|-------|-------|--------|-------|
| `test_janky` | 7 | ✅ Pass | JK-DBG-01, JK-DBG-02 + JK-01..05 |
| `test_page_release` | 5 | ✅ Pass | PRL-05 fixed |
| All other unit suites | 88 | ✅ Pass | 0 regressions |
| `test_arena_integration` | 5 | ✅ Pass | AIT-01 MAX_ARENAS corrected |
| `test_integration` | 8 | ✅ Pass | |
| `test_invariants` | 6 | ✅ Pass | |
| `test_nodepool_growth_validation` | 5 | ✅ Pass | NPV-01 sentinel corrected |
| **Total** | **124** | **✅ 124/124** | **0 failures** |

---

### Lessons Learned

**1. Index-based structures are vulnerable to capacity changes.**  
Raw numeric indices are not pointers — they don't update when the underlying array moves or grows. Any data structure that stores indices into a dynamically-sized pool must have a growth protocol that patches all stored indices. This is not optional. The bug will manifest silently and late.

**2. "Silent wrong answer" bugs are the most dangerous.**  
The phantom B-tree entries didn't crash anything. They returned bad addresses, which caused a skip-list miss, which caused an early return — all without a segfault, assertion failure, or error log. The system appeared healthy. The only observable symptom was a page count that didn't decrease. Without a dedicated test for that specific invariant (PRL-05), this bug could have shipped.

**3. Diagnostic scaffolding belongs in the suite.**  
The instrumentation that revealed the phantom entries — skip-list rc logging, B-tree dynamic-address detection — was written as temporary debug code. We promoted it to permanent regression tests (JK-DBG-01 and JK-DBG-02) instead of deleting it. Future bugs in this subsystem will be caught at the same atomic level that this one was diagnosed.

**4. Test ordering is a design decision.**  
JK-DBG tests must run before JK-01..05 because the JK tests create arenas that persist in SLB0 state. This ordering is documented in a comment in `init_test`. If a future contributor shuffles the test order without reading that comment, the DBG tests will see a dirty SLB0 and produce misleading failures. Ordering constraints are architectural facts and should be documented as such.

**5. Pre-capture before potentially-remapping calls.**  
Any raw pointer into an `mremap`'d region is invalid after the call — this was established in Chapter 9. The `alloc_count` desync fix illustrates the companion rule: any *index* or *offset* computed from pool state before a potentially-remapping call must be captured as a stable value (e.g., the base address from `pn->page_base`) before the call, not re-derived after.

---

**Implemented:** March 11, 2026  
**Commit:** `e2d1b46` — fix(nodepool): reindex btree_node indices after mremap growth  
**Test Results:** 124/124 passing across all suites, valgrind clean  
...
**Current Status:** v0.2.4-nodepool-stability (March 11, 2026)  
**Latest Update:** Chapter 10 - The Janky Instability — NodePool B-tree Index Corruption  
