...
void test_slab_slot_allocation(void) {
    ...
    addr expected_slots_base = sys0_base + SYS0_SLOTS_OFFSET;
    addr expected_slots_end = sys0_base + SYS0_SLOTS_OFFSET + SYS0_SLOTS_SIZE;

    Assert.isTrue(slots_base == expected_slots_base,
                  "Slots base (0x%lx) == sys0 + SYS0_SLOTS_OFFSET [%u] (0x%lx)", slots_base,
                  SYS0_SLOTS_OFFSET, expected_slots_base);
    Assert.isTrue(slots_end == expected_slots_end, "Slots end (0x%lx) == sys0 + %u + %u (0x%lx)",
                  slots_end, SYS0_SLOTS_OFFSET, SYS0_SLOTS_SIZE, expected_slots_end);
...
__attribute__((constructor)) void init_memory_tests(void) {
    ...
    testcase("SYS0: slab slot allocation", test_slab_slot_allocation);
}
#endif
